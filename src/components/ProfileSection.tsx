...
const ProfileSection = () => {
  const { user, isTelegram, loading } = useTelegram();
    ...
    <div className="px-4 py-6 pb-28 max-w-2xl mx-auto">
      ...
      <div className="flex items-center gap-4 p-5 rounded-2xl bg-card card-neon-border mb-6">
        <Avatar className="h-16 w-16 ring-2 ring-primary/30">
          {user!.photo_url ? <AvatarImage src={user!.photo_url} alt={displayName} /> : null}
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-muted-foreground text-xs mt-0.5">ID: {user!.id}</p>
        </div>
      </div>
